...
{
public:
	SquareBlock(int size, int x, int y, int speed, int maxX, int maxY, ofImage* image = nullptr, int index = 0, GridT* grid = nullptr, int num = 0) {
		...
		Blocks.push_back(Block(0, size, size, 0));
		Blocks.push_back(Block(size,size , size, 1));
		Blocks.push_back(Block(size,0 , size, 2));
		Blocks.push_back(Block(0,0, size, 3));
		for (Block i : Blocks) {
